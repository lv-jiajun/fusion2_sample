...
def run_whole_procedure(fig_prefix,df):
    ins_abs_stgs = ['coarse']
    dic_base_path = 'E:/model/JiajunLv/code1/'
...
def TextCnn(df):
    model_store_base_path = 'D:/JiajunLv/code3/'
  ...
  #  label_strs = ['CWE-119  True','CWE-120  True','CWE-469  True','CWE-476  True']
    ...
    for i in range(len(ins_abs_stgs)):
        ...
        for unique_fun_stg in unique_fun_stgs:
            ...
            texts ,embeddings_matrix,fig_prefix = run_whole_procedure(model_name.replace('.h5', ''),df)
            return texts,embeddings_matrix,fig_prefix
