...
def run_whole_procedure(fig_prefix_org, corpus_path1, dic_file_path, func_rep_stg, unique_funs_file, num_classes,
    ...
    """
    global model2
    filename_train= './sample.txt'
    texts, embeddings_matrixcnn, fig_prefix= Part1(filename_train)
    dt_size = len(texts)
    ...
    train_dataset = subDataset(texts, db)
    '''
            ...
            if model_type == 'GAT':  # use the GAT model
                ...
                if node_vec_stg == 'mean':
                    ...
                    model = sumModel(ParameterConfig.GCN_HIDDEN_DIM, ParameterConfig.GCN_HIDDEN_DIM,
                                     2,embeddings_matrixcnn, db, node_vec_stg=node_vec_stg,
...
if __name__ == '__main__':
    corpus_base_path = './testinputs/testcfg/input/'
    dic_base_path = './testinputs/PhaseII-ins2vec-sample/ins2vec_coarse.dic'
    model_store_base_path = './testinputs/testcfg/trained/'
    unique_fun_base_path = './testinputs/UniqueFunList/'
    ...
    for i in range(len(ins_abs_stgs)):
        ...
        for unique_fun_stg in unique_fun_stgs:
           ...
           # dic_file_path = dic_base_path + 'ins2vec_' + ins_abs_stgs[i] + '.dic'
            dic_file_path = 'F:/图神经网络/testinputs/PhaseII-ins2vec-sample/ins2vec_coarse.dic'
